 ...
 * Version: 0.4 SUPER FINAL (wszystkie bugi naprawione)
 * Author: Maciej Kasperek (vonKrappitz)
 * Opis: Pełna wersja z kalibracją live, seriami, pomiarem G i dB, pogodą, mDNS i zapisem kalibracji
 * =======================================================================================
 */
#include <WiFi.h>
#include <WebServer.h>
...
<body>
  ...
  <div class="card">
    <h3>⚙️ Kalibracja Live</h3>
    <div class="input-group"><span>Spust:</span><b id="s_val">0°</b></div>
    <input type="range" id="s_live" min="0" max="180" value="0" oninput="moveLive('spust',this.value)">
    <div style="display:flex;gap:5px"><button class="btn-small" style="flex:1" onclick="saveCal('spust','min')">MIN</button><button class="btn-small" style="flex:1" onclick="saveCal('spust','max')">MAX</button></div>
    <hr style="border:#444;margin:10px 0">
    <div class="input-group"><span>Śruba:</span><b id="p_val">90°</b></div>
    <input type="range" id="p_live" min="0" max="180" value="90" oninput="moveLive('sruba',this.value)">
    <div style="display:flex;gap:5px"><button class="btn-small" style="flex:1" onclick="saveCal('sruba','min')">MIN</button><button class="btn-small" style="flex:1" onclick="saveCal('sruba','max')">MAX</button></div>
  </div>
  ...
  <div class="card">
    ...
    <div class="input-group"><label>Waga [g]:</label><input type="number" id="p_weight" value="3.2" style="width:70px"></div>
  ...
  <script>
    ...
    function toggleAction(){
      fetch('/api/status').then(r=>r.json()).then(d=>{
        if(d.state==='IDLE'){
          let params = `type=${document.getElementById('p_type').value}&weight=${document.getElementById('p_weight').value}&dist=${document.getElementById('p_dist').value}&cadence=${document.getElementById('p_cadence').value}&reqV=${document.getElementById('p_req_v').checked?1:0}`;
...
void setupWebEndpoints() {
  ...
  server.on("/api/set_cal", HTTP_GET, [](){
    String t = server.arg("target"), m = server.arg("mode"); int v = server.arg("val").toInt();
